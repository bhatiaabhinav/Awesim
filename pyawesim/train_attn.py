...
import wandb
from gymenv_dump_all_features import AwesimEnv
...
class ModelArchitecture(nn.Module):
    ...
    def __init__(self, feature_dim: int, dim_out: int, d_model: int = 64, num_encoder_layers: int = 3, num_heads: int = 4):
        super().__init__()

        dim_ff = d_model * 4
        self.proj = nn.Sequential(
            nn.Linear(feature_dim, dim_ff),
            nn.ReLU(),
            nn.Linear(dim_ff, d_model),
            nn.ReLU(),
            nn.LayerNorm(d_model)
        )
        encoder_layer = nn.TransformerEncoderLayer(d_model=d_model, nhead=num_heads, dim_feedforward=dim_ff, batch_first=True, dropout=0)
        self.attention = nn.TransformerEncoder(encoder_layer, num_layers=num_encoder_layers)
        self.post_attn_act = nn.ReLU()
        self.final_fc = nn.Linear(d_model, dim_out)

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        # input x: (batch_size, num_entities, feature_dim)
        x = self.proj(x)  # (batch_size, num_entities, d_model
        x = self.attention(x)  # (batch_size, num_entities, d_model)
        x = x[:, 0, :]  # take the representation corresponding to the agent car (first entity)
        x = self.post_attn_act(x)
        x = self.final_fc(x)  # (batch_size, dim_out)
        return x
...
WANDB_CONFIG = {
    ...
    "experiment_name": "Awesim-Attn",
    "notes": """Attention-based model with transformer encoder layers.""",
    "video_interval": 25,
...
def train_model() -> Tuple[ActorBase, Critic, PPO]:
    ...
    feature_dim = vec_env.single_observation_space.shape[1]     # type: ignore
    action_dim = vec_env.single_action_space.shape[0]       # type: ignore
    print(f"Feature dim: {feature_dim}, Action space dim: {action_dim}")
    actor_dim_out = action_dim * 2 if POLICY_CONFIG["state_dependent_std"] else action_dim

    actor_model = ModelArchitecture(feature_dim=feature_dim, dim_out=actor_dim_out, **NET_CONFIG)
    ...
    critic_model = ModelArchitecture(feature_dim=feature_dim, dim_out=1, **NET_CONFIG)
    ...
    if PPO_CONFIG["cmdp_mode"]:
        cost_critic_model = ModelArchitecture(feature_dim=feature_dim, dim_out=1, **NET_CONFIG)
