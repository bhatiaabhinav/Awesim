...
typedef enum SmartDASDrivingStyle {
    ...
    SMART_DAS_DRIVING_STYLE_NORMAL,          // speed max = at speed limit, follow THW = 3.0s + 3m. Pass/merge thw = 3s + 3m. Yield thw = 3 s (if can stop at all). Full stop at stop signs, proceeds when it's our turn. Stops at yellow lights if can stop comfortably, otherwise ignore. Starts braking for red lights smoothly with up to max preferred deceleration and the stopping trigger is accordingly earlier. Does not stop once impossible to stop (and just coasts). This mode is generally safe and compliant with traffic rules. Defensive driving at intersections (wary of jumpers) with yield thw 3s.
    SMART_DAS_DRIVING_STYLE_DEFENSIVE,       // speed max = 10% below speed limit, follow THW = 4.0s + 4m. Pass/merge thw = 4s + 4m. Yield thw = 4 s (if can stop at all). Does not exercise free-right at intersections. Does not creep while yielding. Otherwise same as NORMAL. This mode is very safe and compliant with traffic rules. Very defensive driving at intersections with yield thw 4s.
