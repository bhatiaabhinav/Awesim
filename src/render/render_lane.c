...
void render_lane_center_line(SDL_Renderer* renderer, const Lane* lane, const SDL_Color color) {
    ...
    if (lane->type == LINEAR_LANE) {
        ...
        thickLineRGBA(renderer, start.x, start.y, end.x, end.y, thickness, color.r, color.g, color.b, color.a);
...
void render_lane_linear(SDL_Renderer* renderer, const LinearLane* lane, const bool paint_lines, const bool paint_arrows) {
    ...
    if (paint_lines) {
        ...
        if (lane->base.adjacent_left && lane->base.adjacent_left->direction != lane->base.direction) {
            thickLineRGBA(renderer, lstart.x, lstart.y, lend.x, lend.y, ythick, yellow.r, yellow.g, yellow.b, yellow.a);
        } else if (!lane->base.adjacent_left) {
            thickLineRGBA(renderer, lstart.x, lstart.y, lend.x, lend.y, wthick, white.r, white.g, white.b, white.a);
        } else {
        ...
        if (!lane->base.adjacent_right) {
            thickLineRGBA(renderer, rstart.x, rstart.y, rend.x, rend.y, wthick, white.r, white.g, white.b, white.a);
    ...
    if (paint_arrows) {
        ...
        for (int i = 0; i < 3; i++) {
            ...
            filledTrigonRGBA(renderer, vx[0], vy[0], vx[1], vy[1], vx[2], vy[2], color.r, color.g, color.b, color.a);
            trigonRGBA(renderer, vx[0], vy[0], vx[1], vy[1], vx[2], vy[2], color.r, color.g, color.b, 255);
        }
    }
}
...
void render_lane_quarterarc(SDL_Renderer* renderer, const QuarterArcLane* lane, const bool paint_lines, const bool paint_arrows) {
    ...
    filledPolygonRGBA(renderer, vx, vy, 2 * (ARC_NUM_POINTS + 1), 128, 128, 128, 255);
    ...
    if (paint_lines) {
        ...
        for (int i = 0; i < ARC_NUM_POINTS; i++) {
            ...
            thickLineRGBA(renderer, p1_left.x, p1_left.y, p2_left.x, p2_left.y,
            ...
            thickLineRGBA(renderer, p1_right.x, p1_right.y, p2_right.x, p2_right.y,
    ...
    if (paint_arrows) {
        ...
        filledTrigonRGBA(renderer, vx[0], vy[0], vx[1], vy[1], vx[2], vy[2],
                         arrow_color.r, arrow_color.g, arrow_color.b, arrow_color.a);
        trigonRGBA(renderer, vx[0], vy[0], vx[1], vy[1], vx[2], vy[2],
