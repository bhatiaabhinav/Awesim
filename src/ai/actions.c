...
//     double merge_duration = duration * 0.3; // default to 30% of the total duration TODO: this can be tuned
...
//     double now = sim_get_time(sim);
//     if (_pass_started_at[cid] <= 0.0) {
//         _pass_started_at[cid] = now; // first invocation
//     }

//     // Abort if we have been stuck for too long.
...
//     // Have we already passed the target with the added buffer? 
...
//             /* Decide whether the ORIGINAL lane lies on our left or right.        *
//             * SituationalAwareness already tells us what is immediately left /   *
//             * right of the ego car, so we just compare those lane-ids with the   *
//             * target’s. If we’re more than one lane away we fall back to a       *
//             * simple numeric heuristic.                                          */
//             const Lane *left_lane  = s->lane_left;   // may be NULL
//             const Lane *right_lane = s->lane_right;  // may be NULL

//             const char *dir_needed;
//             if (left_lane && left_lane->id == car_get_lane_id(target)) {
//                 dir_needed = "left";
//             } else if (right_lane && right_lane->id == car_get_lane_id(target)) {
//                 dir_needed = "right";
//             } else {
//                 LOG_WARN("Car %d: aborting pass – target lane not adjacent.", cid);
//                 _pass_started_at[cid] = 0;
//                 return true;
//             }

//             /* Try to slide back into the original lane. The merge() helper
//             * already contains its own timeout, so we just forward the slice
//             * of the overall manoeuvre budget we allocated above. */
//             if (merge(self, sim, dir_needed, merge_duration)) {
//                 _pass_started_at[cid] = 0;      // clean up timer
//                 return true;                    // finished (success)
//             }
//             return false;                       // still merging
//         }
...
//     // Step 1 – move out to over‑taking lane if still behind target.
//     if (car_get_lane_id(self) == car_get_lane_id(target)) {
//         // Prefer left lane for passing where possible.
//         if (merge(self, sim, "left", merge_duration)) {
//             // after lane change accelerate past target
//             // fall‑through to speed control further below
//         } else {
//             // Could not merge left – try right (multi‑lane roads).
//             merge(self, sim, "right", merge_duration);
//         }
//         return false; // waiting for lane change
//     }

//     // Step 2 – we are in a different lane, speed up (if needed) to pass.
//     MetersPerSecond desired = car_get_speed(target) + speed_buffer;
//     _cruise_to_speed(self, desired);
//     return false; // still overtaking
// }
