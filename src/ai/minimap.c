...
static const RGB COLOR_CAR = {0, 255, 0}; // Green for car

// #define RENDER_DEBUG_GRAPH

#ifdef RENDER_DEBUG_GRAPH
static const RGB COLOR_GRAPH_NODE = {165, 42, 42}; // brown
static const RGB COLOR_GRAPH_EDGE = {200, 200, 200}; // white
#endif

// Helper to set a pixel
static void set_pixel(uint8_t* buffer, int width, int height, int x, int y, RGB color, int alpha) {
    if (x < 0 || x >= width || y < 0 || y >= height) return;
    int idx = (y * width + x) * 3;
    buffer[idx+0] = (uint8_t)((double)(color.r * alpha + buffer[idx+0] * (255 - alpha)) / 255);
    buffer[idx+1] = (uint8_t)((double)(color.g * alpha + buffer[idx+1] * (255 - alpha)) / 255);
    buffer[idx+2] = (uint8_t)((double)(color.b * alpha + buffer[idx+2] * (255 - alpha)) / 255);
}

// Helper to draw a line
static void draw_line(uint8_t* buffer, int width, int height, int x0, int y0, int x1, int y1, RGB color, int thickness, int alpha) {
    ...
    while (1) {
        ...
        for (int tx = -thickness/2; tx <= thickness/2; tx++) {
            for (int ty = -thickness/2; ty <= thickness/2; ty++) {
                set_pixel(buffer, width, height, x0 + tx, y0 + ty, color, alpha);
            }
        }
...
static void draw_filled_square(uint8_t* buffer, int width, int height, int cx, int cy, int size, RGB color) {
    ...
    for (int y = cy - half_size; y <= cy + half_size; y++) {
        for (int x = cx - half_size; x <= cx + half_size; x++) {
            set_pixel(buffer, width, height, x, y, color, 255);
        }
    }
}
...
static void minimap_render_static(MiniMap* minimap, Map* map) {
    ...
    int thickness = (int)(minimap->scale_x * 3.0); // 4 meters wide roads roughly
    if (thickness < 1) thickness = 1;
    // int thickness = 1; // Fixed thickness for better visibility on small minimaps
    ...
    for (int i = 0; i < map->num_lanes; i++) {
        ...
        if (lane->direction == DIRECTION_WEST || lane->direction == DIRECTION_SOUTH) {
            color = COLOR_ROAD_DARKER;
        }

        if (lane->type == LINEAR_LANE) {
            int x0, y0, x1, y1;
            world_to_pixel(minimap, lane->start_point, &x0, &y0);
            world_to_pixel(minimap, lane->end_point, &x1, &y1);
            draw_line(minimap->static_data, minimap->width, minimap->height, x0, y0, x1, y1, color, thickness, 255);
        } else if (lane->type == QUARTER_ARC_LANE) {
            int num_segments = 10;
            double step = (lane->end_angle - lane->start_angle) / num_segments;

            for (int j = 0; j < num_segments; j++) {
                double theta1 = lane->start_angle + j * step;
                double theta2 = lane->start_angle + (j + 1) * step;

                Coordinates p1_world = {
                    lane->center.x + lane->radius * cos(theta1),
                    lane->center.y + lane->radius * sin(theta1)
                };
                Coordinates p2_world = {
                    lane->center.x + lane->radius * cos(theta2),
                    lane->center.y + lane->radius * sin(theta2)
                };
                ...
                draw_line(minimap->static_data, minimap->width, minimap->height, x1, y1, x2, y2, color, thickness, 255);
            }
        }
    }
}
...
MiniMap* minimap_malloc(int width, int height, Map* map) {
    ...
    // Calculate extent from all lanes
    ...
    for (int i = 0; i < map->num_lanes; i++) {
        Lane* lane = map_get_lane(map, i);
        if (!lane) continue;
        Coordinates pts[] = {lane->start_point, lane->end_point};
        for (int j = 0; j < 2; j++) {
        ...
        if (lane->type == QUARTER_ARC_LANE) {
             if (lane->center.x - lane->radius < min_x) min_x = lane->center.x - lane->radius;
             if (lane->center.x + lane->radius > max_x) max_x = lane->center.x + lane->radius;
             if (lane->center.y - lane->radius < min_y) min_y = lane->center.y - lane->radius;
             if (lane->center.y + lane->radius > max_y) max_y = lane->center.y + lane->radius;
        }
    }
...
static void minimap_render_graph(MiniMap* minimap) {
    ...
    for (int u_id = 0; u_id < g->num_nodes; ++u_id) {
        ...
        while (edge) {
            ...
            if (u_node->lane_id == v_node->lane_id && lane_u->type == QUARTER_ARC_LANE) {
                // Draw arc segments along the curve
                double t_start = u_node->progress / lane_u->length;
                double t_end   = v_node->progress / lane_u->length;
                ...
                for (int j = 0; j < segments; j++) {
                    ...
                    int ax, ay, bx, by;
                    world_to_pixel(minimap, pa, &ax, &ay);
                    world_to_pixel(minimap, pb, &bx, &by);
                    draw_line(minimap->data, minimap->width, minimap->height, ax, ay, bx, by, COLOR_GRAPH_EDGE, 1, 255);
                }
            } else {
                Coordinates p2;
                ...
                draw_line(minimap->data, minimap->width, minimap->height, x1, y1, x2, y2, COLOR_GRAPH_EDGE, 1, 255);
            }
    ...
    for (int i = 0; i < g->num_nodes; ++i) {
        ...
        set_pixel(minimap->data, minimap->width, minimap->height, x, y, COLOR_GRAPH_NODE, 255);
    }
}
#endif
...
void minimap_render(MiniMap* minimap, Simulation* sim) {
    if (!minimap || !minimap->data || !minimap->static_data) return;
        ...
        // Render Graph
#ifdef RENDER_DEBUG_GRAPH
    if (minimap->debug_planner && minimap->debug_planner->decision_graph) {
    ...
    if (minimap->marked_path && minimap->marked_path->path_exists) {
        ...
        int thickness = (int)(minimap->scale_x * 6.0);
        ...
        for (int i = 0; i < planner->num_solution_actions; i++) {
            ...
            if (u->lane_id == v->lane_id) {
                ...
                if (lane_u->type == LINEAR_LANE) {
                    ...
                    draw_line(minimap->data, minimap->width, minimap->height, x0, y0, x1, y1, color_path, thickness, 255);
                    ...
                    for (int j = 0; j < segments; j++) {
                        ...
                        draw_line(minimap->data, minimap->width, minimap->height, x1, y1, x2, y2, color_path, thickness, 255);
                    }
                }
            } else {
    ...
    // Draw traffic congestion heatmap using per-edge traffic flow from the path planner.
    // For each lane's graph edge segments, color-code by the smoothed traffic flow speed
    // relative to the lane's speed limit. Works for all lane types (straight and arc).
    PathPlanner* heatmap_planner = minimap->marked_path;
    if (heatmap_planner && heatmap_planner->use_live_traffic_info) {
        Map* map = heatmap_planner->map;
        int thickness = (int)(minimap->scale_x * 3.0);
        ...
        for (int lane_idx = 0; lane_idx < map->num_lanes; ++lane_idx) {
            ...
            int node_count = heatmap_planner->lane_id_to_node_ids_count[lane_idx];
            if (node_count < 2) continue;

            for (int k = 0; k < node_count - 1; k++) {
                int u_id = heatmap_planner->lane_id_to_node_ids_list[lane_idx][k];
                int v_id = heatmap_planner->lane_id_to_node_ids_list[lane_idx][k + 1];
                double seg_start = heatmap_planner->map_nodes[u_id].progress;
                double seg_end   = heatmap_planner->map_nodes[v_id].progress;

                double traffic_speed = heatmap_planner->traffic_flow_per_edge[lane_idx][k];
                double speed_limit = lane->speed_limit;
                if (speed_limit < 1.0) speed_limit = 1.0;

                double flow;
                if (traffic_speed <= 0.0) {
                    // No traffic data for this edge — treat as free flow
                    flow = 1.0;
                } else {
                    flow = traffic_speed / speed_limit;
                }
                if (flow > 1.0) flow = 1.0;
                if (flow < 0.0) flow = 0.0;
                double congestion = 1.0 - flow; // 1.0 = fully congested, 0.0 = free flow

                // Red (congested) to Green (free flow)
                RGB color;
                color.r = 255;
                color.g = 0;
                color.b = 0;
                ...
                Coordinates p1, p2;
                get_lane_point(lane, seg_start, &p1);
                get_lane_point(lane, seg_end, &p2);
                ...
                if (lane->type == QUARTER_ARC_LANE) {
                    // Draw arc segments for proper curve rendering
                    double t_start = seg_start / lane->length;
                    double t_end   = seg_end   / lane->length;
                    double angle_start = lane->start_angle + t_start * (lane->end_angle - lane->start_angle);
                    double angle_end   = lane->start_angle + t_end   * (lane->end_angle - lane->start_angle);

                    int arc_segs = 10;
                    double arc_step = (angle_end - angle_start) / arc_segs;
                    for (int s = 0; s < arc_segs; s++) {
                        double theta1 = angle_start + s * arc_step;
                        double theta2 = angle_start + (s + 1) * arc_step;
                        Coordinates pa = {
                            lane->center.x + lane->radius * cos(theta1),
                            lane->center.y + lane->radius * sin(theta1)
                        };
                        ...
                        world_to_pixel(minimap, pa, &ax, &ay);
                        world_to_pixel(minimap, pb, &bx, &by);
                        draw_line(minimap->data, minimap->width, minimap->height, ax, ay, bx, by, color, thickness, (int)((congestion / 3) * 255)); // Alpha based on congestion
                    }
                } else {
                    draw_line(minimap->data, minimap->width, minimap->height, x1, y1, x2, y2, color, thickness, (int)((congestion / 3) * 255)); // Alpha based on congestion
                }
            }
        }
    }
    ...
    if (minimap->marked_car_id != ID_NULL) {
        ...
        if (car) {
            ...
            // Project onto the car's lane
            Lane* lane = car_get_lane(car, &sim->map);
            if (lane) {
                get_lane_point(lane, car->lane_progress_meters, &car_pos);
            }

            int cx, cy;
            world_to_pixel(minimap, car_pos, &cx, &cy);
            
            // Triangle size based on resolution (e.g., 10 meters)
            double size = minimap->scale_x * 10.0;
