...
#define PHYSICS_HZ 30.0

int main(int argc, char** argv) {
    Meters city_width = meters(1000.0);             // width of the city in meters
    int num_cars = 256;                               // number of cars to simulate
    const Seconds dt = 1.0 / PHYSICS_HZ;                        // time resolution for integration.
    const Seconds control_dt = 1.0 / CONTROL_HZ;
    Simulation* sim = (Simulation*)malloc(sizeof(Simulation));
    LOG_DEBUG("Allocated memory for simulation, size %.2f kilobytes.", sizeof(*sim) / 1024.0);
    awesim_setup(sim, city_width, num_cars, dt, clock_reading(0, 8, 0, 0), WEATHER_SUNNY, false, NULL, control_dt, control_dt);
    ...
    printf("With time resolution dt = %f, control_dt = %f:\n", dt, control_dt);
